...
class Metadata(Base):
  ...
  def game_variants (self, id):
    return self.get (url = self.base_url + 'game-variants/' + str(id))

  def impulses (self):
    return self.get (url = self.base_url + 'impulses')

  def map_variants (self, id):
    return self.get (url = self.base_url + 'map-variants/' + str(id))
  ...
  def requisition_packs (self, id):
    return self.get (url = self.base_url + 'requisition-packs/' + str(id))

  def requisitions (self, id):
    return self.get (url = self.base_url + 'requisitions/' + str(id))
